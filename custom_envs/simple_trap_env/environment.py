import numpy as np
from custom_envs.simple_trap_env import TileMap
import os
import gym
import os
...
class Environment(gym.Env):
    def __init__(self, opt_id="fd_worker_{}".format(os.getpid())):
        self.opt_id = opt_id
        self.rng = np.random.RandomState(0)
        self.action_space = gym.spaces.Discrete(9)

        self.map = TileMap()
        self.map.load_map("custom_envs/simple_trap_env/map.txt")
    ...
    def save(self, filename, folder_path="data/custom_novelty_playbacks/in_progress"):
